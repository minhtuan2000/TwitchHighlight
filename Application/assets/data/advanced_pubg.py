...
keywords = ["lul", "pogchamp", "kappapride", "crosshair", "kekw", "gg"]
antikeywords = []
...
def findhighlights(filename, count, length, offset, keywords = [], antikeywords = [], cutoff = -float('inf'), sort = True, ranges = []):
    ...
    automode = (length <= 0)
    cnt = count
    length = (default_length * 60 // 5) if automode else (int(length * 60) // 5)
    offset = offset // 5
...
highlights, durations = findhighlights(filename, int(sys.argv[4]), int(sys.argv[5]), int(sys.argv[6]), keywords=keywords)
if highlights == []: 
